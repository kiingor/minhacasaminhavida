...
// Variante global e idempotente — varre TODAS as famílias. Pensada para
// execução CLI única após o deploy do bugfix de baixa por mês. Sem auth
// porque é operação admin-global e não-destrutiva (não apaga nem altera
// campos legados; só insere em pagamentosDespesas / recebimentosReceitas
// quando ainda não existe registro para (id, mes)).
export const migrarBaixasGlobal = mutation({
  args: {},
  handler: async (ctx) => {
    const [despesas, receitas] = await Promise.all([
      ctx.db.query("despesas").collect(),
      ctx.db.query("receitas").collect(),
    ]);
...
// Variante por família autenticada (admin). Disponível caso seja necessário
// rodar novamente escopada a uma família específica.
