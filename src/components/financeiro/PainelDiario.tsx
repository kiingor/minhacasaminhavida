 ...
 */
interface PainelDiarioProps {
  /** Familia alvo (consultor visualizando cliente). */
  familyIdAlvo?: string;
  /** Modo leitura: esconde o botão Pagar/Receber. */
  readonly?: boolean;
}

export function PainelDiario({ familyIdAlvo, readonly = false }: PainelDiarioProps = {}) {
  ...
  const lancamentosHoje = useQuery(
    ...
    token ? { sessionToken: token, familyIdAlvo } : "skip"
            ...
            >
              ...
              {!readonly && (
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs font-medium px-2 h-7 rounded-md text-ink-700 hover:bg-cream-100 disabled:opacity-50 transition-colors"
                >
                  {processando === item.id ? (
